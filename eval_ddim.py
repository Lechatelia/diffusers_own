...
class InferenceConfig:
    ...
    output_dir = 'ddim-eval-128'  # the model namy locally and on the HF Hub
...
model_name = 'ddim-butterflies-128'
pipeline = DDIMPipeline.from_pretrained(model_name).to("cuda")
...
def evaluate(config, epoch, pipeline):
    ...
    image_grid.save(f"{test_dir}/{config.num_inference_steps:04d}_{config.eta:.1f}.png")
